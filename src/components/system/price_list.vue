...
export default {
    ...
    methods:{
        ...
        ok () {
            this.axios.post(this.http_url+'/system/action_system?action=del&table=price_list&id='+this.data_list[this.id].id)
                .then(res=>{
                    console.log(res)
                    if(res.data==1)
        ...
        handleSubmit (name) {
                ...
                this.axios.post(this.http_url+'/system/action_system?action=put&table=price_list', data)
                .then((res)=>{
                    if(res.data>0){
                        ...
                        this.getSystem.then(res=>{
                            this.data_list=res.data.price_list=this.data_list.concat(Ndata)
                        })
                    }
                }).catch(()=>{
                     this.$Notice.error({
    ...
    created () {
         this.getSystem.then(res=>{
                this.data_list=res.data.price_list
            })
    }
}
</script>
